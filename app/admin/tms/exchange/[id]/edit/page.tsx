...
// String helper: db value → form string
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setForm({
          title: s(data.title),
          origin_company: s(data.origin_company),
          origin_address: s(data.origin_address),
          origin_city: s(data.origin_city),
          origin_postal_code: s(data.origin_postal_code),
          origin_country: s(data.origin_country),
          origin_lat: data.origin_lat ?? null,
          origin_lng: data.origin_lng ?? null,
          dest_company: s(data.dest_company),
          dest_address: s(data.dest_address),
          dest_city: s(data.dest_city),
          dest_postal_code: s(data.dest_postal_code),
          dest_country: s(data.dest_country),
          dest_lat: data.dest_lat ?? null,
          dest_lng: data.dest_lng ?? null,
          load_date_from: s(data.load_date_from),
          load_date_to: s(data.load_date_to),
          unload_date_from: s(data.unload_date_from),
          unload_date_to: s(data.unload_date_to),
          vehicle_type: s(data.vehicle_type),
          body_type: s(data.body_type),
          length_m: s(data.length_m),
          weight_kg: s(data.weight_kg),
          ldm: s(data.ldm),
          pallet_count: s(data.pallet_count),
          volume_m3: s(data.volume_m3),
          adr_class: s(data.adr_class),
          temp_min: s(data.temp_min),
          temp_max: s(data.temp_max),
          goods_description: s(data.goods_description),
          pricing_mode: s(data.pricing_mode) || "open",
          price_amount: s(data.price_amount),
          currency: s(data.currency) || "EUR",
          payment_terms_days: s(data.payment_terms_days),
          notes: s(data.notes),
        });
      } catch (err) {
    ...
    load();
  }, [adminSession?.id, offerId, supabase]);
      ...
      <div className="border-b border-border/40 bg-card/60 px-4 py-3 sm:px-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push(`/admin/tms/exchange/${offerId}`)}
