  ...
  if (!cod && clientNume) {
    ...
    await conn.query(
      `
        ...
        `,
      [newCod, clientNume.substring(0, 64), (factura.clientCIF || "").substring(0, 20), contCli],
    );

    cod = newCod;
    log(
      `   ✅ Client nou creat in Saga: ${clientNume} | COD: ${newCod} | CIF: ${factura.clientCIF || "-"}`,
    );
  }
...
async function sagaUpdateFactura(factura, existing, payment) {
  ...
  try {
    ...
    if (payment && typeof payment.remainingAmount === "number") {
      ...
      neachitat = Math.max(0, r2(total - platit));
    }
