...
/**
 * Import Incasari — bank receipt import from a CAMT.053 (BT GO) statement.
 *
 * Steps:
 *   1. Upload — operator picks the .xml statement file.
 *   2. Preview — only incoming credits (CRDT) are shown. Each row displays the
 ...
 *   3. Done — summary of how many payments were recorded.
 *
...
export function ReceiptImportDialog({ open, onOpenChange, adminId, onImported }: Props) {
  ...
  async function runPreview() {
    ...
    try {
      ...
      toast({
        title: "Nu am putut analiza fisierul",
  ...
  async function commit() {
    ...
    if (items.length === 0) {
      toast({ title: "Nimic de inregistrat", description: "Selectati cel putin o factura." })
      return
    }
    setCommitting(true)
    try {
      ...
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-hidden p-0">
        <DialogHeader className="border-b border-border px-6 py-4">
          <DialogTitle className="flex items-center gap-2">
            ...
            Import Incasari (extras CAMT.053)
          </DialogTitle>
          <DialogDescription>
            Incarcati extrasul bancar Banca Transilvania. Sistemul potriveste incasarile cu clientii si
            facturile, iar dvs. confirmati inainte de inregistrare.
          </DialogDescription>
        </DialogHeader>
            ...
            >
              ...
              <div>
                <p className="font-medium text-foreground">
                  {file ? file.name : "Selectati fisierul CAMT.053 (.xml)"}
                </p>
                <p className="text-sm text-muted-foreground">
                  {file ? "Apasati Analizeaza pentru a continua" : "Click pentru a alege fisierul"}
                </p>
              </div>
            </button>
            <input
            ...
            <div className="flex flex-wrap items-center gap-2 border-b border-border bg-muted/30 px-6 py-3 text-sm">
              {preview.account.iban && (
                <span className="text-muted-foreground">
                  Cont: <span className="font-medium text-foreground">{preview.account.iban}</span>
                  {preview.account.currency ? ` (${preview.account.currency})` : ""}
                </span>
              )}
              <span className="ml-auto flex flex-wrap gap-3 text-xs">
                <span className="text-emerald-700">{preview.summary.matched} potrivite</span>
                <span className="text-amber-700">{preview.summary.review} de verificat</span>
                <span className="text-red-700">{preview.summary.unmatched} nepotrivite</span>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-background">
                  <tr className="border-b border-border text-left text-xs uppercase tracking-wide text-muted-foreground">
                    <th className="px-4 py-2 font-medium">Platitor</th>
                      ...
                      <tr key={row.id} className="border-b border-border align-top last:border-0">
                        <td className="px-4 py-3">
                          <div className="font-medium text-foreground">
                            {row.credit.debtorName ?? "Necunoscut"}
                          </div>
                          ...
                          {row.partner && (
                            <div className="mt-1 text-xs text-foreground">
                              Client: <span className="font-medium">{row.partner.partnerName}</span>
                            </div>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          {isDuplicate ? (
                            <span className="text-xs text-muted-foreground">Ignorata</span>
                          ) : (
                            <Select
                              value={selections[row.id] ?? SKIP}
                              onValueChange={(v) => setSelections((s) => ({ ...s, [row.id]: v }))}
                            >
                              <SelectTrigger className="w-[260px]" aria-label="Alege factura">
                                <SelectValue placeholder="Alege factura" />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value={SKIP}>Nu inregistra (skip)</SelectItem>
                                {opts.map((o) => (
                                  <SelectItem key={o.id} value={o.id}>
                                    {(o.invoiceNumber ?? "fara numar") +
                                      " · " +
        ...
        {step === "done" && result && (
          <div className="flex flex-col items-center gap-3 px-6 py-10 text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground">Incasari inregistrate</h3>
            <p className="text-sm text-muted-foreground">
              {result.recorded} plata{result.recorded === 1 ? "" : "i"} inregistrate
              {result.skipped ? `, ${result.skipped} ignorate` : ""}.
            </p>
            {result.errors.length > 0 && (
              <div className="mt-2 w-full rounded-md border border-red-200 bg-red-50 p-3 text-left text-xs text-red-800">
                <p className="mb-1 font-medium">Erori:</p>
          ...
          {step === "preview" && (
            <>
              ...
              <Button onClick={commit} disabled={committing || confirmableCount === 0}>
                ...
                Inregistreaza {confirmableCount} incasari
              </Button>
            </>
          )}
          {step === "done" && <Button onClick={() => onOpenChange(false)}>Inchide</Button>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
